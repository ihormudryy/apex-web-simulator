...
The line is `P_i = C_i + o_i * N_i`, and the objective is total SQUARED curvature
`Σ|D_j|²` where `D_j = P_{j-1} - 2P_j + P_{j+1}`.

`o_i` appears in `D_{i-1}`, `D_i` **and** `D_{i+1}`, so the gradient has three terms:

```
grad_i = (D_{i-1} - 2·D_i + D_{i+1}) · N_i
```

Projected gradient descent on that, clamped into the corridor each sweep. The stencil is
biharmonic with spectral radius 16, so the step must stay below `2/16 = 0.125`; measured, 0.15
diverges (the line blew out to 43 km) and 0.02 is stable.

**Do not use a per-station closed form solving `D_i · N_i = 0`.** That accounts for only one of
the three terms, and its fixed point is a degenerate line that rides the wall through every
corner.

**Do not measure success with total ABSOLUTE curvature.** Measured on this centerline,
`Σ|κ| = 20.11` and `total_turning / ds = 20.11` — the same number, because total turning of a
closed loop is a topological invariant (Fenchel). It cannot be reduced by choosing a line, and
an apex-cutting line is *shorter*, which shrinks `ds` and makes `Σ|κ|` larger. Measure `Σκ²`
and peak `κ` instead: at step 0.02 over 60k sweeps the measured ratios against the centerline
were 0.883 and 0.859, the line 4 m shorter, with 310 offset sign changes.
...
test('the line reduces squared and peak curvature', () => {
  // NOT absolute curvature: that is total turning over mean spacing, a
  // topological invariant a shorter line makes WORSE. Squared curvature is the
  // objective; peak curvature is what sets the minimum corner speed.
  const line = buildRacingLine(cl.samples);
  const flat = buildRacingLine(cl.samples, { iterations: 0 });
  assert.ok(lineCurvatureTotal(line) < lineCurvatureTotal(flat) * 0.92,
    'relaxation did not reduce squared curvature by at least 8% (measured 11.7%)');
  const peak = l => l.curvature.reduce((m, k) => Math.max(m, k), 0);
  assert.ok(peak(line) < peak(flat) * 0.92,
    'relaxation did not reduce peak curvature by at least 8% (measured 14.1%)');
});
...
/**
 * Total SQUARED curvature — the quantity the relaxation actually minimises.
 *
 * Not the absolute sum: measured on this centerline that is 20.11, and total
 * turning over mean spacing is also 20.11, because they are the same thing.
 * Total turning of a closed loop is fixed no matter which line you take, and a
 * line that cuts apexes is shorter, so it shrinks the spacing and scores worse.
 */
...
export function driveAi(ai, car, line, out, rival = null) {
  ...
  // The line's corner speeds are quoted at LINE_LAT_G; scale them to this
  // driver. Only the curvature-derived values — `topSpeed` is set by power and
  // drag, not by grip, so rescaling it by a CORNERING budget is meaningless.
  // Measured when the scale was applied to the whole target instead: club
  // reached 55.3 m/s against its configured 70 (79%), pro 75.1 of 85, ace 87.3
  // of 92 — every straight on every level silently detuned, and invisible to an
  // ordering test because all three shrink monotonically.
  const scale = Math.sqrt(level.latG / LINE_LAT_G);
  let target = level.topSpeed;
  for (let d = 0; d < horizon; d += step) {
    const i = (here + d) % n;
    const cornerV = line.speed[i] * scale;
...
```js
  queryWheel(x, z, out, hint = this._wheelHint) {
    ...
    // Published so a caller keeping its own cursor can advance it without
    // paying for a second ring search. `out` is per-wheel (the kernel passes
    // `car.surfaces[i]`), so this cannot be clobbered by another wheel.
    out.index = q.index;
    // ... body unchanged
  }
```
